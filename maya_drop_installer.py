...
MAYA_APP_DIR = pc.mel.eval("getenv MAYA_APP_DIR;")
MAYA_VERSION = pc.versions.shortName()
USER_SCRIPT_DIR = Path(MAYA_APP_DIR) / MAYA_VERSION / "scripts"
CG3_SETUP_PATH = Path(__file__).parent / "maya" / "scripts" / "cg3"
CG3_SETTINGS_DIR = "cg3settings"
SETUP_KEY = "CG_SCRIPTS_PATH"

sys.path.append(str(CG3_SETUP_PATH.parent))
maya_gui = importlib.import_module("cg3.ui.maya_gui")
...
def create_userSetup():
    file_content = [
        ...
        "import importlib",
        "import pymel.core as pc",
        f"sys.path.append(r'{str(CG3_SETUP_PATH.parent)}')",
        'importlib.import_module("cg3")'
    ]
...
def import_setup():
    importlib.import_module("cg3.setup")


def show_results():
    ratios = (30, 200)
    padding = 3
    with pc.window(title="CG3 Installation Summary") as win:
