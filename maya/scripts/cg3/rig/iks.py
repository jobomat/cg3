...
def ik_stretch(ik_handle, pole=None, lock=False, max_stretch=5, name=None, parent_measure_loc=True):
    ...
    div_node, s_e_div_attr = scalefactor_node(
        start_loc, end_loc, div_node, name="{}_start_to_end".format(name),
    ...
    if pole and lock:
        div_node, s_p_div_attr = scalefactor_node(
            start_loc, pole, div_node, name="{}_start_to_pole".format(name),
            initial_len=hierarchy_len(joints[0], joints[1]))
        div_node, p_e_div_attr = scalefactor_node(
            pole, end_loc, div_node, name="{}_pole_to_end".format(name),
...
class StretchyIK():
    ...
    def gui(self):
        ...
        with pc.window(win_id, title="Stretchy IK Setup", wh=[window_width, window_height]) as self.win:
            with pc.frameLayout(borderVisible=False, labelVisible=False, marginWidth=7, marginHeight=7):
                with pc.columnLayout(adj=True, rs=6):
                    self.name_textField = pc.textFieldGrp(
                        label="Name: ", text="", cw2=[40, 2], adj=2,
                    ...
                    self.add_lock_checkBox = pc.checkBox(
                        label="Add Lock Feature (Pole Required)", value=True,
                        onCommand=self.check_prerequisites, offCommand=self.check_prerequisites
                    )
                    self.autoparent_checkBox = pc.checkBox(
                        label="Autoparent Measure Locator", value=True,
                    ...
                    with pc.rowLayout(nc=2, cw2=[70, 60], adj=2):
                        ...
                        self.max_stretch_floatField = pc.floatField(value=5.0, minValue=1.0, pre=1)

                    pc.separator(h=1)

                    self.global_scale_checkBox = pc.checkBox(
                        label="Add Global Scale Compensation Attribute", value=False,
                        annotation="Supply a global scale attribute for scaleable stretch-setups.",
                        onCommand=self.toggle_scale_enable, offCommand=self.toggle_scale_enable
                    )

                    self.global_scale_textField = pc.textFieldButtonGrp(
                        label="", editable=False, bl="Set", cw3=(0, 100, 40),
                        adj=2, placeholderText="Select Attribute in Channel Box and:", enable=False,
                    ...
                    self.create_button = pc.button(
                        label="Make IK Stretchy", enable=False,
                        c=self.create_setup
                    )
                    pc.separator(h=1)
                    self.hint_text = pc.text(label="")

        self.script_jobs.append(pc.scriptJob(e=("SelectionChanged", self.check_prerequisites)))
    ...
    def toggle_scale_enable(self, *args):
        self.global_scale_textField.setEnable(not self.global_scale_textField.getEnable())
        self.check_prerequisites()
    ...
    def check_prerequisites(self, *args):
        ...
        if self.global_scale_checkBox.getValue():
            if self.global_scale_attr is None:
                self.hint("Set a global scale attribute\nor disable scale feature checkbox.")
                self.create_button.setEnable(False)
                return
