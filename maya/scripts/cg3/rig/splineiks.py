...
def create_spline_ik(
    start_joint: pc.nodetypes.Joint, end_joint: pc.nodetypes.Joint,
    name: str = None, curve_spans: int = 4
...
def setup_squash_stretch(ik_handle: pc.nodetypes.IkHandle, scale_factor_attr: pc.Attribute, name: str):
    ...
    pc.keyTangent(anim_curve_node, e=True, absolute=True,
                  t=len(joint_list), inAngle=-50)
    ...
    for i, joint in enumerate(joint_list, 1):
        frame_cache = pc.createNode(
            "frameCache", n=f"{name}_{joint.name()}_fcache")
...
class SplineIKRig:
    ...
    def __init__(self, ik_handle: pc.nodetypes.IkHandle):
        ...
        self._tag = "spline_ik_handle"  # do not change or backwards compatibiliy will break
        self.initial_dropoff_rate: float = 4.0
        # add necessary attributes to ik_handle
        for attr in ["control_joints", "controls", "stretch_div", "skin_cluster", "curve_info"]:
    ...
    def bind_curve(self):
        sc = pc.skinCluster(
            *self.control_joints, self.curve, bindMethod=0, 
    ...
    def adjust_dropoff(self, jnt, dropoff_delta: float):
        ...
        pc.skinCluster(
            self.skin_cluster, e=True, inf=jnt, dropoffRate=val
        )

    def reset_dropoff(self):
        for jnt in self.control_joints:
            jnt.dropoff_rate.set(self.initial_dropoff_rate)
            pc.skinCluster(self.skin_cluster, e=True, inf=jnt, dr=self.initial_dropoff_rate)

    def add_volume(self):
        sel = pc.selected()
        setup_squash_stretch(
            self.ik_handle, self.scale_factor_attr, self.prefix
        )
        pc.select(sel)
    ...
    def is_stretchy(self):
        ...
        if not crv:
            return False
        return True if crv.attr("worldSpace[0]").listConnections(type="curveInfo") else False
    ...
    def control_joints(self):
        return sorted(
            self.ik_handle.control_joints.listConnections(),
            key=lambda x: x.order.get()
        )
...
class SplineIKManager:
        ...
        with pc.window(self.win_id, title="Spline IK Manager") as self.win:
            with pc.formLayout() as main_fl:
                ...
                with pc.columnLayout(adj=True, rs=5, enable=False) as self.edit_cl:
                    with pc.rowLayout(nc=3, adj=1, cw3=(col1, col2, col3)) as self.name_rl:
                    ...
                    pc.separator(h=1)
                    with pc.rowLayout(nc=3, adj=1, cw3=(col1, col2, col3)) as self.span_rl:
                        ...
                        self.spans_button = pc.button(
                            label="Change", w=col3, enable=False,
                            c=self.change_number_of_spans
                        )
                    pc.separator(h=1)
                    with pc.rowLayout(nc=3, adj=1, cw3=(col1, col2, col3)) as self.ctrl_count_rl:
                        ...
                        self.number_of_controls_textField = pc.textField(
                            ...
                            alwaysInvokeEnterCommandOnReturn=True,
                            ec=self.change_number_of_spans, text="3"
                        )
                        self.number_of_controls_button = pc.button(
                            label="Create", w=col3, enable=False, c=self.create_controllers
                        )
                    pc.separator(h=1)
                    with pc.columnLayout(adj=True) as self.controls_cl:
                        with pc.rowLayout(nc=2, cw2=(1, 2 * self.w_btn), cat=(2,"both",0), adj=1):
                            pc.text(label="Joint", font="boldLabelFont", align="left")
                            pc.text(label="Rigidity", font="boldLabelFont", align="center")
                        with pc.scrollLayout(childResizable=True, bgc=(.2,.2,.2), h=110):
                            with pc.columnLayout(adj=True, visible=False, rs=2) as self.controls_list_cl:
                                pass
                        with pc.rowLayout(nc=2, cw2=(1, 2 * self.w_btn - 2), adj=1):
                            pc.text(label="")
                            pc.button(
                                label="Reset", w=2 * self.w_btn,
    ...
    def list_spline_ik_handles(self):
        return [h for h in pc.ls(type="ikHandle") if type(h.ikSolver.get()) is pc.nodetypes.IkSplineSolver]
    ...
    def ik_menu_changed(self, item):
        ...
        if self.current_handle:
            ...
            self.number_of_controls_textField.setText(
                len(self.rig.controls) or "3"
            )
    ...
    def update_controls_list(self):
        ...
        for i, jnt in enumerate(self.rig.control_joints):
            ...
            with pc.rowLayout(nc=3, cw3=(1, self.w_btn, self.w_btn), adj=1, parent=self.controls_list_cl):
                pc.text(label=jnt.name(), align="left")
                pc.button(
                    label="-", w=self.w_btn, h=h_btn,
