...
class BackgroundPlayblast(SettingsManagerMixin):
            ...
            with pc.formLayout(numberOfDivisions=100) as main_fl:
                with pc.columnLayout(adj=True, rs=padding) as self.main_cl:
                    self.scene_tfg = file_chooser_button(
                        "Scene", 1, " Choose... ", text=pc.system.sceneName(),
                        okCaption="Choose", tfb_kwargs={'cw3': (60, 100, 50)}
                    )
                    with pc.popupMenu(parent=self.scene_tfg):
                        pc.menuItem(
                            "Current Scene",
                            c=self.set_scene_to_current_scene
                        )
                    pc.separator(h=2)
                    self.renderdir_tfg = file_chooser_button(
                        "Render Dir", 3, " Choose... ", text=self.get_current_image_dir(),
                        okCaption="Set", tfb_kwargs={'cw3': (60, 100, 50)}
                    )
                    ...
                    with pc.rowLayout(nc=5, cw=(2, 30), adj=2, cat=(2, "right", 4)):
                        with pc.optionMenuGrp(label="Camera", cw=(1, 60), adj=2, cal=(1, "left")) as self.camera_optionMenuGrp:
                        ...
                        self.image_name_textFieldGrp = pc.textFieldGrp(
                            label="Image Name", cw=(1, 75), adj=2, cal=(1, "right"),
                            text=self.get_current_scene_name()
                        )
                        with pc.popupMenu(parent=self.image_name_textFieldGrp):
                            pc.menuItem(
                                "Current Scene Name",
                                c=pc.Callback(self.set_image_name_to_current_scene_name)
                            )
                        pc.text(label="Pad")
                        self.padding_intField = pc.intField(value=4, w=20)
                        with pc.optionMenuGrp(label="File Format", cw=(1, 77), cat=(1, "left", 10)) as self.file_format_optionMenuGrp:
                            pc.menuItem("png")
                            pc.menuItem("jpg")
                            pc.menuItem("exr")
                    ...
                    with pc.frameLayout(label="FFMPEG Encoding Settings", bgc=(0.07, 0.22, 0.25), mh=4, mw=4, collapsable=True):
                        with pc.columnLayout(adj=True):
                            ...
                            with pc.rowLayout(nc=4, adj=4):
                                ...
                                self.fps_intField = pc.intField(value=self.get_fps(), w=30)
                                with pc.popupMenu(parent=self.fps_intField):
                                    pc.menuItem(
                                        "From project Settings", c=pc.Callback(self.set_fps)
                                    )
                                self.quality_intSliderGrp = pc.intSliderGrp(
                                    label="Quality", v=65, min=0, max=100,
                                    cw3=(50, 30, 100), f=True
                                )
                                self.ffmpeg_exe_tfg = file_chooser_button(
                                    "FFMPEG exe", 1, "...", text=self.get_ffmpeg_exe(),
                                    okCaption="Choose",
                            ...
                            with pc.rowLayout(nc=3, ct3=("left", "left", "left") ,co3=(0,0,4)):
                                with pc.optionMenuGrp(label="Presets", cw=(1, 60), cal=(1, "left"), cc=self.change_drawtext) as self.burnin_presets_MenuGrp:
                                    for item in self.settings.burnin_presets.keys():
                                        pc.menuItem(item)
                                pc.button(label="Save New Preset", c=self.save_drawtext_preset)
                                pc.button(label="Use as Default Preset", c=self.save_default_preset)
                            with pc.horizontalLayout():
                                with pc.columnLayout(adj=True):
                                    pc.text(label="LEFT", align="center",
                                            h=20, font="boldLabelFont")
                                    add_left_btn = pc.button(label="Add Line")
                                    with pc.columnLayout(adj=True) as self.burnin_cols["left"]:
                                        pass
                                with pc.columnLayout(adj=True):
                                    pc.text(label="CENTER", align="center",
                                            h=20, font="boldLabelFont")
                                    add_center_btn = pc.button(label="Add Line")
                                    with pc.columnLayout(adj=True) as self.burnin_cols["center"]:
                                        pass
                                with pc.columnLayout(adj=True):
                                    pc.text(label="RIGHT", align="center",
                                            h=20, font="boldLabelFont")
                                    add_right_btn = pc.button(label="Add Line")
                                    with pc.columnLayout(adj=True) as self.burnin_cols["right"]:
                                        pass
                                add_left_btn.setCommand(
                                    pc.Callback(self.add_drawtext, self.burnin_cols["left"])
                                )
                                add_center_btn.setCommand(
                                    pc.Callback(self.add_drawtext, self.burnin_cols["center"])
                                )
                                add_right_btn.setCommand(
                                    pc.Callback(self.add_drawtext, self.burnin_cols["right"])
                                )
                    with pc.popupMenu(parent=self.resolution_intFieldGrp):
                        pc.menuItem(
                            "From Render Settings",
                            c=pc.Callback(
                                self.set_resolution_from_rendersettings)
                        )
                        pc.menuItem(
                            "50%",
    ...
    def get_current_image_dir(self):
        image_dir = Path(pc.workspace.path) / \
            pc.workspace.fileRules.get("images", "")
    ...
    def get_quality(self):
        return int(remap_value(
            self.quality_intSliderGrp.getValue(), 0, 100, 63, 0
        ))
    ...
    def build_hw2_rendersettings(self):
        ...
        with pc.rowLayout(nc=3, parent=self.renderer_fl):
            ...
            with pc.optionMenuGrp(label="AA Oversampling", cw=(1, 100)) as self.hw2_oversampling_optionMenuGrp:
                pc.menuItem("2")
                pc.menuItem("4")
                pc.menuItem("8")
                pc.menuItem("16")
            self.hw2_oversampling_optionMenuGrp.setValue("16")
            self.hw2_ao_checkBox = pc.checkBox(value=False, label="SS Ambient Occlusion")
    ...
    def save_default_preset(self, *args):
        self.settings.default_preset = self.burnin_presets_MenuGrp.getChildren()[1].getValue()
        self.settings.save()
    ...
    def run(self, actions):
        ...
        if "encode" in actions:
            if not self.ffmpeg_exe_tfg.getText():
                pc.confirmDialog(
                    title="Info missing", message="Please specify where to find 'ffmpeg.exe'.")
            else:
