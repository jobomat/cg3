...
from cg3.ui.windows import WindowManager, WinWrapper
...
class ModuleManager(WinWrapper):
    def __init__(self):
        ...
        self.filter_textFieldGrp = pc.textFieldGrp(
            label="Filter Modules", text=self.filter, 
            tcc=self.set_filter, parent=self.top, adj=2
        )
        ...
        pc.button(label="Refresh", parent=self.bottom,
                  c=self.build_module_list_layout)
    ...
    def build_module_list_layout(self, *args):
        ...
        for mod in sys.modules.keys():
            if self.filter in mod:
                with pc.rowLayout(parent=self.module_list_layout, nc=3, adj=2, cal=(2, "left")) as row:
                    pc.checkBox(w=20)
                    pc.text(label=mod)
                    pc.button(label="Delete", w=40, c=pc.Callback(self.del_module, mod, row))
    ...
    def gui(self):
        ...
        with pc.window(self.window_name, title="cg3 Dev Tools") as win:
            with pc.columnLayout(adj=True):
                ...
                pc.button(label="Module Manager",
                          c=pc.Callback(ModuleManager))
